...
// Function to calculate the sum of numbers from i down to 1 using recursion
function sum(i) {
  // Base case: If i is 1, return 1 (stopping condition for recursion)
  if (i === 1) {
    return 1;
  }

  // Recursive case: Add the current number (i) to the sum of (i - 1)
  return i + sum(i - 1);
}

// Calling the function with an initial value of 5
const result = sum(5);
...
//****** */ kono ekta function er vitor theke shei function k jodi arekbar call kora hoy shetake bole recursion.
...
// Function to find products whose names match the search term
function matchedProducts(products, search) {
  const matched = []; // Initialize an empty array to store matched products
  // Loop through each product in the products array
  for (const product of products) {
    // Convert both product name and search term to lowercase for case-insensitive search
    if (product.name.toLowerCase().includes(search.toLowerCase())) {
      matched.push(product); // Add the matching product to the matched array
    }
  }
  return matched; // Return the array containing all matched products
}
// Sample array of products with id, name, and price
const products = [
  { id: 1, name: "Xaomi phone One night", price: 19000 },
  { id: 2, name: "iphone", price: 19000 },
  { id: 3, name: "macbook Air", price: 119000 },
...
// Call the function with "phone" as the search keyword
const result = matchedProducts(products, "phone");
// Output the list of matched products in the console
console.log(result);
